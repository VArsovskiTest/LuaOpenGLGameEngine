...
namespace LuaOpenGLGameEngine
{
    public class GameEngine : GameWindow
    {
        ...
        protected override void OnRenderFrame(FrameEventArgs e)
        {
            ...
            // _currentScene = new GenericScene(_luaProcessor.ProcessLuaQuery<Dictionary<string, ActorRGB>>("render_scene"));
            // _currentScene = GenericScene.FromLuaTable(_luaProcessor.ProcessLuaQuery<Dictionary<string, ActorRGB>>("initGame"));
            LuaTable luaResult = _lua.DoString("return render_scene()").First() as LuaTable;
            _currentScene = GenericScene.FromLuaTable(luaResult);
            RedrawScene(_currentScene);

            _lua.DoString("current_scene = {}"); // Reset for next frame
            SwapBuffers();
        }
        ...
        void RedrawScene(GenericScene sceneData)
        {
            ...
            {
                bool isHovered = actor.Id == hoveredActor?.Id;
                bool isSelected = actor.Id == selectedActor?.Id;
                ...
                bool shouldHighlight = isHovered || isSelected;

                if (shouldHighlight &&
                    actor is IPlaceable placeable &&
                    actor is ISizable sizable)
                {
                    float brightness = isSelected ? 1.5f : 1.2f;
        ...
        protected override void OnMouseDown(MouseButtonEventArgs e)
        {
            ...
            {
                var pickedActor = _mouseClickHandler.SelectActor(
                    _currentScene,
                    e.Position.X,
                    e.Position.Y,
                    (int)_viewport.Width,
        ...
        protected override void OnMouseMove(MouseMoveEventArgs e)
        {
            ...
            ActorRGB? actorHovered = _mouseClickHandler.SelectActor(_currentScene, e.Position.X, e.Position.Y, (int)_viewport.Width, (int)_viewport.Height);

            if (actorHovered != null) hoveredActor = actorHovered;

            // // Cursor change?
            // if (actorHovered != null)
